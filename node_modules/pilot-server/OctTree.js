module.exports = (function() {
	function OctTree(options) {
		this.root = new Node(options.rootNode);
	}
	...
	OctTree.prototype.retrieve = function(item) {
		var rv = this.root.retrieve(item).slice(0);
		if(rv.length == 0) throw new Error("Tree empty.");
		else return rv;
	}

	function Node(information) {
		information = information || {};
		if(!information.bounds) {
			throw new Error("Node created without boundaries.");
		}
	...
	Node.prototype.insert = function(item) {
		if(	!item.hasOwnProperty("position") ||
			...
			!item.position.hasOwnProperty("z")) {
			/// @todo: simplify check by making sure item.position is instanceof (as-yet unwritten) Position class
			throw new Error("Invalid item.  Must have 'position' set.");
		}
