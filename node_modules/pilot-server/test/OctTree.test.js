...
// obscenely large number of nodes (update if 'obscenely large' changes its definition)
var LARGE_NUMBER = 1024;
...
describe("OctTree",function() {
	var o;
	beforeEach(function() {
		// do not think 'o.clear()' in an afterEach block will suffice here.  The tree may be corrupted during a failing test, causing
		// subsequent tests to fail inexplicably and throw red herring failures at the test runner
		o = new OctTree({
			rootNode: {
				bounds: {
					x:0,
	...
	describe("#insert",function() {
		...
		it("should support an arbitrarily large number of nodes",function() {
			for(var i = 0; i < LARGE_NUMBER; i++) {
				(function() { 
					o.insert(createUniqueFakeNode(i));
				}).should.not.throw();
			}
		});
	});
	...
	describe("#clear",function() {
		it("should wipe all nodes out of the tree",function() {
			for(var i = 0; i < 8; i++) o.insert(createUniqueFakeNode(i));
			o.clear();
